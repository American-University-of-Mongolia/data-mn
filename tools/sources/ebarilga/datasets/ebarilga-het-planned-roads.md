# Dataset: HET planned roads

## Identification

- **ID**: `ebarilga-het-planned-roads`
- **Source**: `ebarilga`
...
- **Host**: `ebarilga`
- **Geometry**: Polygon
- **Features**: 2362 (counts 2026-09-13)
- **Attributes**: unprobed
- **Raw files**: `tools/sources/ebarilga/raw/ebarilga-ddp_road.{geojson,csv,meta.json}`
- **Note**: raw GeoJSON is over 5 MB and stays on this server (see source.md Large-file policy).

## Title

- **EN**: HET planned roads — Ulaanbaatar (eBarilga geoportal)
- **MN**: ХЕТ-ний төлөвлөлтийн зам — Улаанбаатар (eBarilga геопортал)

## Description

HET planned roads in Ulaanbaatar from the eBarilga geoportal
(2362 Polygon features). Attribute richness not yet surveyed; check the CSV headers.
...
python3 query_api.py --layer ddp_road          # live count vs catalog
python3 fetch_data.py --layer ddp_road --output ../../tools/sources/ebarilga/raw --with-attributes
```
